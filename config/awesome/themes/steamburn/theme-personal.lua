...
-- lain related
theme.layout_txt_termfair           = "[te]"
theme.layout_txt_centerfair         = "[tec]"
theme.layout_txt_cascade            = "[ca]"
theme.layout_txt_cascadetile        = "[cat]"
theme.layout_txt_centerwork         = "[cw]"
theme.layout_txt_centerworkh        = "[chw]"
...
local bat = lain.widget.bat({
    settings = function()
        ...
        if bat_now.status == 'Charging' then bat_perc = "P:" .. bat_perc end
...
local net = lain.widget.net({
    ...
    settings = function()
        if net_now.state == "up" then
            widget:set_markup(markup.font(theme.font, markup(gray, " R ") .. net_now.received .. markup(gray, " S ") .. net_now.sent .. " "))
        else net_state = "Off"
...
function theme.at_screen_connect(s)
    ...
    if s.index == 1 then
        gears.wallpaper.maximized(theme.wallpaper, s, true, {x = 10, y = 10})
    else
        gears.wallpaper.centered(theme.wallpaper2, s, theme.bg_normal)
    end

    -- Tags and it's layout dummys
    awful.tag(awful.util.tagnames, s, awful.layout.layouts)
    awful.layout.set(awful.layout.suit.fair, s.tags[1])
    awful.layout.set(awful.layout.suit.fair, s.tags[2])
    awful.layout.set(awful.layout.suit.fair, s.tags[3])
    awful.layout.set(awful.layout.suit.fair, s.tags[4])
    awful.layout.set(awful.layout.suit.fair, s.tags[5])
    awful.layout.set(awful.layout.suit.fair.horizontal, s.tags[6])
    awful.layout.set(awful.layout.suit.floating, s.tags[7])
    awful.layout.set(awful.layout.suit.floating, s.tags[8])
    ...
    -- Create the wibox
    s.mywibox = awful.wibar({ position = "top", screen = s, height = 38, bg = theme.bg_normal, fg = theme.fg_normal })
