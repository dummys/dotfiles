...
run_once({ "urxvtd","/usr/bin/sxhkd", "unclutter -root", "xcalib  -d :0 -s 0 ~/.config/profile_xps9550.icc", "xset -dpms; xset s off", "xrdb -merge ~/.Xresources", "xrandr --dpi 288", "libinput-gestures-setup start" })

-- high dpi scaling
require("lgi").GLib.setenv("GDK_SCALE", "2", true)
require("lgi").GLib.setenv("QT_AUTO_SCREEN_SCALE_FACTOR", "1", true)
require("lgi").GLib.setenv("GDK_DPI_SCALE", "0.5", true)
...
--- Tags definition and name
awful.util.terminal = terminal
awful.util.tagnames = { "w3b", "h4ck", "t3rm", "IRC", "c0de", "stuff", "RE", "VM" }
...
-- Lock the screen
    awful.key({ modkey }, "l", function() os.execute("i3lock -c000000 && /usr/local/sbin/lock_that_shit.sh") end),

    -- Non-empty tag browsing  removed by  dummys
...
--        end),
    ...
    awful.key({ altkey, "Control" }, "Up",
        function ()
            awful.spawn.with_shell("mpc toggle || ncmpc toggle")
            beautiful.mpd.update()
        end),
    awful.key({ altkey, "Control" }, "Down",
        function ()
            awful.spawn.with_shell("mpc stop || ncmpc stop")
            beautiful.mpd.update()
        end),
    awful.key({ altkey, "Control" }, "Left",
        function ()
            awful.spawn.with_shell("mpc prev|| ncmpc prev")
            beautiful.mpd.update()
        end),
    awful.key({ altkey, "Control" }, "Right",
        function ()
            awful.spawn.with_shell("mpc next|| ncmpc next")
            beautiful.mpd.update()
        end),
    awful.key({ altkey }, "0",
        function ()
...
awful.rules.rules = {
    ...
    -- fix  for maximized chromium,firefox bug - ADDED by dummys
    { rule = { },
        properties = { },
        callback = function(c)
    ...
    -- MODIFICATION DUMMYS, remove fucking titlebar
    -- Titlebars
